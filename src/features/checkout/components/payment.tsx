...
const Payment = ({ isConfirm, hotel }: Props) => {
  ...
  const reservationId = useAppSelector(
    state => state.reservationSlice.currentReservation,
  );
  const globalCurrency = useAppSelector(
    state => state.globalSlice.searchGlobal.currency.code,
  );

  // api
  const { data: dataPayments, isLoading: isLoadingPayment } =
    useGetPaymentMethodsQuery({});
  ...
  const handleConfirmPayment = async () => {
    try {
      ...
      const res = await createPayment(request).unwrap();
      if (res) {
        console.log(res);

        globalThis.location.href = res.payment_link.payment_link.href;
      }
    } catch (error) {
      console.log(error);
    }
      ...
      <div className="mt-6">
        ...
        >
          {dataPayments?.map(pay => (
            <div key={pay.id}>
              ...
              <Label
                htmlFor={pay.id}
                className="flex cursor-pointer flex-col items-center justify-between space-y-1 rounded-lg border-2 border-muted bg-popover p-4 hover:bg-accent hover:text-accent-foreground peer-data-[state=checked]:border-blue-400 [&:has([data-state=checked])]:border-blue-400"
              >
          ...
          <div key={'credit'}>
            ...
            <Label
              htmlFor={'credit'}
              className="flex cursor-pointer flex-col items-center justify-between space-y-1 rounded-lg border-2 border-muted bg-popover p-4 text-center hover:bg-accent hover:text-accent-foreground peer-data-[state=checked]:border-blue-400 [&:has([data-state=checked])]:border-blue-400"
            >
              <div className="relative flex aspect-video items-center justify-center">
                {/* <img
                                    src={"/credit-card.png"}
        ...
        <div className="flex flex-1 justify-end pt-8">
          <ButtonLoading
            ...
            className="focus:ring-primary-600 relative inline-flex h-auto items-center justify-center rounded-lg px-4 py-3 text-sm font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:bg-opacity-70 dark:focus:ring-offset-0 sm:px-6 sm:text-base"
          >
            Confirm and pay
          </ButtonLoading>
        </div>
